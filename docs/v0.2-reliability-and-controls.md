...
| Ultra-wide / tele camera catalog UI | Out unless a cheap cycle-through falls out of flip robustness. |
...
Cap: 8 phones. Insert beyond cap evicts the least-recently-`last_seen` and prints a one-line warning. One stream at a time still applies.
...
4. **Report** `camera: "back"|"front"` as an optional field on `POST /pair` and `POST /reconnect`. `GET /status` echoes the last reported value. Do not put `camera` on the QR payload. `POST /telemetry` (not `/health`, which collides with `GET /healthz`) can later refresh it along with battery/thermal; it is not required for this field to be populated.

**Out of scope:** a multi-lens picker (ultra-wide, tele, USB camera gadgets), Camera2 Interop for per-lens zoom curves, torch. If `availableCameraInfos` yields multiple back cameras, cycling them on repeated flips is an acceptable opportunistic extra **only if** it fits in the same PR as (1)–(3) without new UI. It is not a release blocker.
...
`Camera` is last value from `/pair` or `/reconnect` (optional `camera` on those bodies). It must not depend on telemetry.

Optional `POST /telemetry` (LAN, requires `resume_token` in JSON body; **not** `/health`, which would collide with `GET /healthz`) lets the phone push battery / thermal / dropped-frame counters that `docs/TECHNICAL_DESIGN.md` already listed as a v0.1 wish and never built. Keep the body small and the interval 5 s. Land it in the degraded-network PR if at all; `last_rtp_ms` alone is enough for reconnect.
...
| Control | `linux-cli/internal/control/server.go` (`Media`, `AutoApprove`, `/reconnect`, richer `/status`, loopback `/trust`, `/telemetry`) |
...
| `POST` | `/telemetry` | `resume_token` | **New, optional.** Phone battery/thermal/drops. Do not name this `/health`. |
...
- `POST /telemetry` is local-only and optional. Do not add crash reporters.
...
1. **Trust expiry** (e.g. 90 days unused) vs explicit-revoke-only? This draft is explicit-revoke-only.
2. **Opportunistic multi-back-camera cycle** on flip — include in the camera PR or leave on the floor?
3. **Should `phonecam start` print “Trusted phones can tap Reconnect”** even when the store is empty? (This draft: only when `len(phones) > 0`. Waiting `status` line is `trusted reconnect allowed`, not accepted.)
4. **`POST /telemetry` in the degraded-network PR or later?** Not needed for reconnect; camera facing comes from `/pair`/`/reconnect`.

Cross-phone takeover, first-start of gst, SPS/PPS copy, `SetAllow` learn reset, and `NewGate(0)` are **decided**. None of the remaining questions block the planning PR.
...
12. **USB, torch, latency harness are not in this cycle.** USB stays on the v1.0 line. Torch and the measurement lab go to Later. `docs/BENCHMARKS.md` remains a template.

13. **Protocol version stays 1.** Additive JSON fields (`laptop_id`, richer `/status`, optional `camera` on `/pair`/`/reconnect`) plus new paths (`/reconnect`, `/telemetry`). Mixed-version QR pairing keeps working.
...
- **Files/components:** `VideoEncoder` + new `BitrateController` (JVM tests); **bitrate table lives here only** (HIGH/MEDIUM/LOW × 15/30); `/status.request_keyframe` driven by gate `LastPacket`; `pipeline.go` `h264parse config-interval=-1` + `pipeline_test.go`; optional `POST /telemetry`.
...
- **Description:** Honest remainder of “camera switching.” No multi-lens UI. No torch. Does not depend on `/telemetry`.
