...
5. **Controls that are not already shipped** — front/back flip and the 720p/540p/360p selector exist; v0.2 finishes robustness and bitrate-per-preset (already shipped). The 15 fps Home toggle is **cancelled** (user decision 2026-08-18). Mid-stream renegotiation is explicitly out.

USB, torch, audio, tray, systemd, and a measurement harness are **cancelled**, not deferred.
...
| Item | Disposition |
| --- | --- |
| USB / ADB transport | Out. Not a product goal. |
| Torch toggle | Out. Not a product goal. |
| Latency measurement harness | Out. Do not fill `docs/BENCHMARKS.md` with a lab. |
| `POST /telemetry` (battery/thermal/drops) | Out (decided 2026-08-17). Not in v0.2 PRs. |
...
| mDNS / LAN scan / extra discovery protocols | Out. Last-known control URL only. |
| Audio, tray, systemd user service | Out. Not a product goal. |
...
- Switch Home quality to Low, Reconnect: laptop reports 640×360@30 and the picture stays live. Changing quality **during** REC is still impossible (selector is Home-only).
...
Same SharedPreferences file `"phonecam"` already used for `phone_id`, `stream_quality`:
...
Goal: **stay live**. Not: measure glass-to-browser ms. Not: add retransmission (NACK/RTX adds delay and a second datagram path).
...
#### Sender: bitrate table (owned by PR 7; already shipped. 15 fps rows unused; no Home toggle)
...
**Shipped:** Home `qualityToggleGroup` (720p / 540p / 360p), persisted `stream_quality`, committed at scan as `selectedQuality.toProfile()`, announced in `POST /pair` `video`, receiver sized to that profile. Live caption `live_quality_caption` already prints height and fps. All presets are 16:9 landscape 30 fps (`StreamQualityTest`). Comments in `MainActivity` state the canvas is “Never swapped or renegotiated.” **Bitrate-per-preset** (table in Workstream D) shipped in PR 7.

**Not shipped, leftover of this workstream:** none in v0.2. Quality is pre-session: changing the toggle during REC is impossible because the selector lives on Home. Changing it and then using Reconnect is a **new** stream: new `/pair` or `/reconnect` `video`, possible `gst-launch` restart if WxH changed.

**Cancelled (user decision 2026-08-18):** the 15 fps Home toggle (planned PR 9). Not needed. Do not add `stream_fps` or replace the decorative `quality_fps` = `"30 fps"` string. The 15 fps bitrate table rows shipped in PR 7 may stay; they are unused.
...
- Auto quality based on thermal/SoC. Tempting for the A03; too much policy. LOW is the manual escape hatch.
...
`POST /telemetry` is **out of product** (decided 2026-08-17). Battery / thermal / dropped-frame counters are not a goal. `last_rtp_ms` is enough for reconnect and PLI.
...
| `POST` | `/telemetry` | — | **Out of product.** Do not add this path. |
...
| `stream_fps` | no | **Not used.** 15 fps Home toggle is cancelled. |
...
**Decision:** last-known URL. No mDNS.
...
8. Camera polish. The 15 fps Home toggle is **cancelled** (user decision 2026-08-18).
...
4. **`POST /telemetry` — Decided (2026-08-17):** cancelled. Not a product goal. Camera facing comes from `/pair` / `/reconnect`. `GET /healthz` is unchanged.
5. **15 fps Home toggle — Decided (2026-08-18):** not needed. PR 9 is cancelled. Workstream F leftover is bitrate-per-preset only (already shipped in PR 7).
...
8. **No mid-stream canvas/FPS renegotiation.** Home selector + bitrate table. A quality change takes effect on the next pair/reconnect and may restart `gst-launch` only if WxH changed. The 15 fps Home toggle is cancelled.

9. **Front/back flip is done.** v0.2 camera work is persist, single-camera safety, and status reporting — not a new picker.

10. **Resolution selector is done.** v0.2 leftover was bitrate-per-preset (4 Mbps @ 360p is a bug); that table shipped in PR 7. The 15 fps Home toggle is **cancelled** (user decision 2026-08-18). The decorative `quality_fps` = `"30 fps"` string stays.
...
12. **USB, torch, latency harness, and `/telemetry` are not product goals.** `docs/BENCHMARKS.md` remains a template. Front/back flip only; no extra-lens cycle. Trust is explicit-revoke-only.
...
- **Files/components:** `docs/ROADMAP.md`, `docs/v0.2-reliability-and-controls.md` (this document), `docs/TECHNICAL_DESIGN.md` (trusted pairing becomes a v0.2 goal; USB remains a non-goal; note that pinning is specified but not yet live), `docs/PRD.md` (same scope corrections only). Do **not** restore `docs/HANDOFF_CLAUDE.md`. Do **not** invent benchmark rows.
- **Depends on:** nothing.
- **Description:** Rewrite the v0.2 bullet list to: auto-reconnect, persistent trusted pairing, degraded-network behavior, camera-switch polish, resolution leftovers (bitrate-per-preset; 15 fps toggle cancelled). USB, torch, latency tooling, and `POST /telemetry` are out of product. Record that flip and the 720p/540p/360p selector already shipped.
...
- **Description:** **On the v0.2 tag critical path.** Lower bitrate and faster IDR when the encoder is dropping or `last_rtp_ms` is stale. Skip-frame fallback at the floor. Do not grow the jitterbuffer. Do not add a measurement harness. Owns `VideoEncoder`’s target-bitrate constructor argument. PR 9 (15 fps toggle) was cancelled 2026-08-18.
...
### PR 9: feat: add a 15 fps pre-session option — **CANCELLED / out of v0.2**

- **Title:** `feat: add a 15 fps pre-session option`
- **Branch:** `feat/quality-fps` (do not open)
- **Status:** **Cancelled 2026-08-18.** User decision: the 15 fps Home toggle is not needed. Workstream F leftover in v0.2 is bitrate-per-preset only, and that already shipped in PR 7. Do not add `stream_fps` or replace `quality_fps`.
- **Depends on:** n/a.
- **Description:** Was: pre-session 15/30 fps toggle. Receiver already accepts whatever `video` the phone reports. No 15 fps Home control.
...
PR 1 (docs)
  └─ PR 2 (gate: loopback udpsrc, 4MB rcvbuf, delayed 1s IP learn, ACL counters)
       ...
       └─ PR 7 (degraded network + bitrate table)   [v0.2 tag critical path]
            └─ PR 9 (15 fps Home toggle) **CANCELLED 2026-08-18**
