...
5. **Controls that are not already shipped** — front/back flip and the 720p/540p/360p selector exist; v0.2 finishes robustness and bitrate-per-preset (already shipped). The optional 15 fps Home toggle is **Later / unscheduled**, not v0.2 or v0.3 (user decision 2026-08-18). Mid-stream renegotiation is explicitly out.
...
#### Sender: bitrate table (owned by PR 7; already shipped. 15 fps rows stay for a Later toggle)
...
**Cancelled / out of v0.2 (user decision 2026-08-18):** the 15 fps Home toggle (planned PR 9). Not needed. Do not add `stream_fps` or replace the decorative `quality_fps` = `"30 fps"` string this cycle. Move the toggle to **Later (unscheduled)**, not v0.3. The 15 fps bitrate table rows shipped in PR 7 may stay; they are unused until a later toggle.
...
| `stream_fps` | no | **Not in v0.2.** Later, if a 15 fps Home toggle ships. |
...
8. Camera polish. The 15 fps Home toggle is **cancelled / out of v0.2** (user decision 2026-08-18); it is Later, not v0.3.
...
5. **15 fps Home toggle — Decided (2026-08-18):** not needed. PR 9 is cancelled / out of v0.2. Workstream F leftover is bitrate-per-preset only (already shipped in PR 7). Put the toggle in Later (unscheduled), not v0.3.
...
8. **No mid-stream canvas/FPS renegotiation.** Home selector + bitrate table. A quality change takes effect on the next pair/reconnect and may restart `gst-launch` only if WxH changed. The 15 fps Home toggle is Later, not v0.2.

9. **Front/back flip is done.** v0.2 camera work is persist, single-camera safety, and status reporting — not a new picker.

10. **Resolution selector is done.** v0.2 leftover was bitrate-per-preset (4 Mbps @ 360p is a bug); that table shipped in PR 7. The 15 fps Home toggle is **cancelled / out of v0.2** (user decision 2026-08-18) and is Later, not v0.3. The decorative `quality_fps` = `"30 fps"` string stays.
...
- **Depends on:** nothing.
- **Description:** Rewrite the v0.2 bullet list to: auto-reconnect, persistent trusted pairing, degraded-network behavior, camera-switch polish, resolution leftovers (bitrate-per-preset; 15 fps toggle later cancelled). Move USB to v1.0 (already listed there). Add a **Later** bucket for torch, latency measurement tooling, and `POST /telemetry`. Record that flip and the 720p/540p/360p selector already shipped.
...
- **Status:** **Cancelled 2026-08-18.** User decision: the 15 fps Home toggle is not needed. Workstream F leftover in v0.2 is bitrate-per-preset only, and that already shipped in PR 7. Move the toggle to **Later (unscheduled)**, not v0.3. Do not add `stream_fps` or replace `quality_fps` this cycle.
- **Depends on:** n/a.
- **Description:** Was: pre-session 15/30 fps toggle. Receiver already accepts whatever `video` the phone reports, including 15 fps, if a Later PR adds the control.
...
PR 1 (docs)
  └─ PR 2 (gate: loopback udpsrc, 4MB rcvbuf, delayed 1s IP learn, ACL counters)
       ...
       └─ PR 7 (degraded network + bitrate table)   [v0.2 tag critical path]
            └─ PR 9 (15 fps Home toggle) **CANCELLED 2026-08-18 — Later, not v0.3**
